...
async function generateTopicWithGemini() {
  ...
  const prompt = `אתה אסטרטג SEO של Site-Control — חברת התקנות ישראלית עם חנות אונליין של מצלמות אבטחה, מקליטים, אינטרקום ובקרת כניסה (Hikvision, Uniview, Reolink, VisionNet, Tenda), וגם מצלמות סולאריות 4G (Reolink GO Plus ו-PTZ Solar) לאתרי בנייה, חקלאות ושטחים בלי חשמל. טיימלאפס אינו חלק מהאתר הזה (זה אתר אחר, timelapseit.co.il), אל תציע נושאי טיימלאפס או תיעוד בנייה.
...
3. רלוונטי למה שהחברה באמת מוכרת: מצלמות אבטחה לבית ולעסק, מקליטים, אינטרקום, בקרת כניסה, ומצלמות סולאריות 4G לאתרים ללא חשמל/אינטרנט.
4. ספק גם slug באנגלית: מילים באנגלית בלבד, מופרדות במקפים, ללא שנה (היא תתווסף אוטומטית).
5. חשוב: אין לכתוב על טיימלאפס או תיעוד בנייה בכלל. עדיף נושאים על: מחיר ועלות, השוואת דגמים, סיפורי מקרה ספציפיים, שאלות טכניות ממוקדות, תחזוקה, התקנה.
...
async function generateBlogContent(topic) {
  ...
  // טיימלאפס אינו חלק מהאתר הזה יותר: אם הוא עולה בכל זאת, מפנים לאתר האחות
  const timelapseLinkInstruction = `\n## טיימלאפס:
- אל תכתוב סעיפים על טיימלאפס או תיעוד בנייה. אם זה עולה במשפט אחד, הפנה ל-<a href="https://timelapseit.co.il" target="_blank" rel="noopener">timelapseit.co.il</a> ואל תרחיב.`;
...
Site-Control היא חברת התקנות עם חנות אונליין של מצלמות אבטחה (/store). הפוסט הזה עוסק בקו המצלמות הסולאריות 4G (Reolink GO Plus ו-PTZ Solar) לאתרי בנייה, חקלאות ושטחים בלי חשמל ואינטרנט.
...
- סיים עם CTA: ייעוץ בווצאפ או השארת פרטים ב-/contact; אם רלוונטי גם קישור לחנות /store
