...
export function recommend(a: Answers): Recommendation {
  ...
  // 1. אתר בנייה / שטח פתוח: מצלמות סוללה 4G עם סים
  if (a.place === "site") {
    ...
    wired[0].qty = n;
    return {
      headline: "לאתר בלי חשמל ואינטרנט: מצלמות סוללה עם סים 4G",
      intro: "מצלמות קוויות צריכות חשמל, כבל רשת ומקליט. באתר בנייה או בשטח פתוח זה בדרך כלל לא קיים, לכן שם מתקינים מצלמות Reolink עם סוללה, פאנל סולארי וסים 4G, שמקליטות לכרטיס ושולחות התראות לנייד.",
      items,
      ext: [{ href: "/use-cases", label: "איך זה עובד באתרי בנייה ובחקלאות", why: "דוגמאות, מיקום ההתקנה וחבילת סים" }],
      notes: ["צריך סים עם חבילת גלישה (כ-10 עד 30GB לחודש). אנחנו עוזרים לבחור ולהגדיר.", "התקנה על עמוד או פיגום על ידינו: לפי הצעת מחיר לאתר.", "יש באתר חשמל ואינטרנט יציב? אז מערכת קווית זולה יותר לטווח ארוך. ראו את החלופה למטה."],
      alternatives: [
        { title: "שטח גדול במצלמה אחת", items: [item("reolink-trackmix-lte", 1, "מצלמה סולארית 4G עם זום", "שתי עדשות: רואה את כל השטח ומתקרבת אוטומטית למי שנכנס")], why: "כשצריך לזהות פרטים במרחק בלי להוסיף מצלמות" },
        { title: "יש חשמל ואינטרנט באתר", items: wired, why: "מצלמות 4K עם הרתעה (אור וסירנה) ומקליט, מתאים למחסן ציוד קבוע או למשרד אתר" },
      ],
    };
  }
    ...
    return {
      ...
      alternatives: indoor
        ? [{ title: "לחוץ במקום לפנים", items: [item("ct3", 1, "מצלמת Wi-Fi צינור", "Full HD, ראיית לילה צבעונית עד 30 מטר, אזעקת אור וקול")], why: "כניסה או מרפסת" },
           { title: "בלי שקע חשמל בכלל", items: [item("reolink-argus-eco", 1, "מצלמת Wi-Fi בסוללה", "3MP, סוללה שמחזיקה שבועות, אפשר להוסיף פאנל סולארי")], why: "כשאין שקע ליד נקודת ההתקנה" }]
