...
const store_catalog_1 = require("../data/store-catalog");
exports.questions = [
    {
        id: "place",
...
const bySlug = (slug) => {
    const p = store_catalog_1.storeProducts.find((x) => x.slug === slug);
    if (!p)
        throw new Error("finder: missing product " + slug);
    return p;
};
const item = (slug, qty, role, why) => ({ product: bySlug(slug), qty, role, why });
const camCount = (c) => (c === "1" ? 1 : c === "2-4" ? 4 : c === "5-8" ? 8 : c === "9-16" ? 16 : c === "17+" ? 24 : 4);
const budgetMax = (b) => (b === "low" ? 1500 : b === "mid" ? 3500 : Infinity);
...
function pickIpCamera(place, priority, cheaper = 0) {
...
function pickNvr(n, priority) {
    ...
    if (n <= 16)
        return item("ds-7616nxi-k2", 1, "מקליט", "16 ערוצים, 2 דיסקים לזמן הקלטה ארוך, AcuSense מסנן התראות");
    return item("nvr302-32b-iq", 1, "מקליט", "32 ערוצים עד 12MP, 2 דיסקים, במחיר של מקליט 16 ערוצים אצל אחרים");
}
function pickAnalog(priority, cheaper = 0) {
    const picks = {
        color: [
...
function pickHybridRecorder(n, priority) {
    if (n <= 4)
        return priority === "cheap"
    ...
    if (n <= 16)
        return item("ids-7216hqhi-m1-xt", 1, "מקליט היברידי", "16 ערוצים אנלוגיים + 8 IP, AcuSense");
    return item("ids-7232hqhi-m2-xt", 1, "מקליט היברידי", "32 ערוצים אנלוגיים + 8 IP, 2 דיסקים");
}
function pickKit(place, priority, budget) {
...
function recommend(a) {
    ...
    if (a.place === "site") {
        const wired = [pickIpCamera("home", "deter"), pickNvr(n)];
        wired[0].qty = n;
        return {
    ...
    if (a.existing === "analog") {
        ...
        if (t > max)
            cam = cheapestStep((s) => pickAnalog(a.priority, s), n);
        if (t > max && a.priority !== "cheap") {
            rec = pickHybridRecorder(n, "cheap");
        }
        return {
    ...
    if (a.existing === "ipnvr") {
        ...
        cam.qty = n;
        if (totalOf([cam]).sum > max)
            cam = cheapestStep((s) => pickIpCamera(a.place, a.priority, s), n);
    ...
    if (n <= 4 && (a.priority === "simple" || a.budget === "low" || a.priority === "cheap")) {
        ...
        const diy = [pickIpCamera(a.place, "cheap"), pickNvr(4)];
        diy[0].qty = n;
        return {
    ...
    const extra = [];
    if (a.place === "business" && n >= 5)
        extra.push(item("ds-2cd2955g0-isu-1-05mm", 1, "מצלמה פנורמית לחלל המכירה", "180° במצלמה אחת: רואים את כל החנות בלי נקודות מתות"));
    if (a.place === "building" && n >= 5)
        extra.push(item("ids-2cd7a46g2-p-izhsy-8-32mm", 1, "מצלמת לוחיות לחניון", "קוראת לוחיות רישוי עד 100 מטר, גם בלילה"));
    const items = [cam, rec, ...extra];
    const t = totalOf(items);
    if (a.budget && a.budget !== "any" && t.sum > max)
        notes.push(`המפרט שבחרת עובר את התקציב שסימנת. אפשר להתחיל בפחות מצלמות ולהוסיף אחר כך: המקליט כבר תומך.`);
    notes.push("המקליט מסופק בלי דיסק קשיח. דיסק Surveillance 4TB מספיק לחודש הקלטה של 8 מצלמות.");
    if (!rec.product.slug.includes("8p"))
        notes.push("להזנת המצלמות צריך מתג PoE. אנחנו מתאימים מתג לפי מספר המצלמות והמרחקים.");
...
function whatsappText(a, r) {
    const label = (q) => exports.questions.find((x) => x.id === q)?.options.find((o) => o.value === a[q])?.label;
    const lines = [
