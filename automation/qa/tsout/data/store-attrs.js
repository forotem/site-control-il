...
exports.tierLabel = exports.mpLabel = exports.isRecorder = exports.isCamera = exports.CAMERA_KINDS = exports.aiLabel = exports.audioLabel = exports.nightShort = exports.nightLabel = exports.kindLabel = exports.storeAttrs = void 0;
exports.attrsOf = attrsOf;
exports.fitLine = fitLine;
exports.tierOf = tierOf;
exports.productsByKind = productsByKind;
...
exports.nightLabel = { ir: "אינפרא-אדום: שחור-לבן בלילה", color: "צבע מלא 24/7 (אור לבן)", hybrid: "היברידי: צבע או אינפרא לפי בחירה" };
exports.nightShort = { ir: "אינפרא בלילה", color: "צבע בלילה", hybrid: "צבע/אינפרא" };
exports.audioLabel = { none: "ללא שמע", mic: "מיקרופון", "two-way": "שמע דו-כיווני" };
exports.aiLabel = { none: "זיהוי תנועה רגיל", "human-vehicle": "מסנן אדם/רכב", acusense: "AcuSense: אדם/רכב בלי התראות שווא" };
function attrsOf(p) {
    const slug = typeof p === "string" ? p : p.slug;
    return exports.storeAttrs[slug] || { kind: "camera" };
}
exports.CAMERA_KINDS = ["bullet", "turret", "dome", "fisheye", "pt", "lpr", "camera", "wifi-pt", "wifi-bullet"];
const isCamera = (a) => exports.CAMERA_KINDS.includes(a.kind);
exports.isCamera = isCamera;
const isRecorder = (a) => ["nvr", "dvr", "xvr"].includes(a.kind);
exports.isRecorder = isRecorder;
const mpLabel = (mp) => !mp ? "" : mp >= 12 ? `${mp}MP 4K+` : mp >= 8 ? "8MP 4K" : mp === 6 ? "6MP" : mp === 5 ? "5MP" : mp === 4 ? "4MP" : mp === 3 ? "3MP 2K" : `${mp}MP`;
exports.mpLabel = mpLabel;
...
function fitLine(p) {
    ...
    switch (a.kind) {
        case "bullet":
            if (a.varifocal)
                return "מרחקים משתנים: שער, כניסה רחוקה, חניון";
            if ((a.range || 0) >= 60)
                return "שטח גדול: מגרש, חצר ארוכה, מחסן חיצוני";
            if (a.deter)
                return "אזור רגיש שצריך הרתעה: חניה, מחסן, דלת אחורית";
        ...
        case "dvr":
        case "xvr": return `שדרוג מערכת קואקס קיימת, עד ${a.channels} מצלמות`;
...
function tierOf(p) {
    const a = attrsOf(p);
    if ((0, exports.isCamera)(a)) {
        if (a.deter || ((a.mp || 0) >= 8 && a.wideSensor) || a.kind === "lpr")
            return "pro";
        if (a.wideSensor || a.night === "color" || a.varifocal || (a.mp || 0) >= 8)
            return "plus";
        return "basic";
    }
    return "plus";
}
exports.tierLabel = { basic: "בסיסי", plus: "משודרג", pro: "פרימיום" };
function productsByKind(kind) { return store_catalog_1.storeProducts.filter((p) => attrsOf(p).kind === kind); }
